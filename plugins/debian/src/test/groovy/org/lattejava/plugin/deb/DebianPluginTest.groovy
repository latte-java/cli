...
class DebianPluginTest {
  ...
  void beforeMethod() {
    ...
    project.version = new org.lattejava.version.Version("1.0")
...
line3""")
    ...
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("conffiles"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/expected/conffiles"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("md5sums"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/expected/md5sums"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("control"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/expected/control"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("postinst"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/scripts/postinst"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("postrm"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/scripts/postrm"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("preinst"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/scripts/preinst"))).trim())
    assertEquals(new String(Files.readAllBytes(controlExplodeDir.resolve("prerm"))).trim(), new String(Files.readAllBytes(projectDir.resolve("src/test/scripts/prerm"))).trim())
