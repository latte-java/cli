...
public class DefaultRuntimeConfigurationParserTest extends BaseUnitTest {
  ...
  public void parse() throws Exception {
    ...
    assertEquals(config.targets, List.of("foo", "bar"));
  ...
  public void parseCommand() {
    ...
    assertEquals(config.targets, List.of());
    ...
    assertEquals(config.targets, List.of("clean", "init"));

    // Unknown word is a target
    config = parser.parse("build");
    assertNull(config.command);
    assertEquals(config.targets, List.of("build"));
  }
}
